...
export default function FormCliente() {
  ...
  async function enviarEnderecos(clienteID) {
    if (endereco.length === 0) {
      console.log('Nenhum endereço para enviar.');
      return;
    }

    try {
      for (const end of endereco) {
        await axios.post(`http://localhost:8080/api/cliente/endereco/${clienteID}`, end);
          ...
          <div style={{ marginTop: '4%' }}>
            <Form>
              ...
              <Form.Group>
                <Form.Input fluid label="Fone Celular" width={6}>
                  <InputMask mask="(99) 9999.9999" value={foneCelular} onChange={(e) => setFoneCelular(e.target.value)} />
                </Form.Input>
            ...
            {endereco.map((address, index) => (
              <div key={index} style={{ marginBottom: '2%' }}>
                ...
                <Form>
                  <FormGroup>
                    <Form.Input required fluid label="Rua" width={8} value={address.rua} onChange={(e) => atualizarEndereco(index, 'rua', e.target.value)} />
                    <Form.Input required fluid label="Numero" width={6} value={address.numero} onChange={(e) => atualizarEndereco(index, 'numero', e.target.value)} />
                    <Form.Input fluid label="CEP" width={6}>
                      <InputMask mask="99999-999" value={address.cep} onChange={(e) => atualizarEndereco(index, 'cep', e.target.value)} />
                    </Form.Input>
                  </FormGroup>
                  <FormGroup>
                    <Form.Input required fluid label="Bairro" width={6} value={address.bairro} onChange={(e) => atualizarEndereco(index, 'bairro', e.target.value)} />
                    <Form.Input required fluid label="Cidade" width={6} value={address.cidade} onChange={(e) => atualizarEndereco(index, 'cidade', e.target.value)} />
                    <Form.Input required fluid label="Estado" width={6} value={address.estado} onChange={(e) => atualizarEndereco(index, 'estado', e.target.value)} />
                  </FormGroup>
                  <Form.Input required fluid label="Complemento" value={address.complemento} onChange={(e) => atualizarEndereco(index, 'complemento', e.target.value)} />
                </Form>
              </div>
            ))}
            ...
            <Button type="button" inverted circular icon labelPosition="left" color="green" floated="right" onClick={adicionarEndereco}>
              <Icon name="plus" />
